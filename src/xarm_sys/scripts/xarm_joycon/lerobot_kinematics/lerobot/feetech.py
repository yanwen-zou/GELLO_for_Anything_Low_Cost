...
def convert_degrees_to_steps(degrees: Union[float, np.ndarray], models: Union[str, List[str]]) -> np.ndarray:
...
class FeetechMotorsBus:
    ...
    def apply_calibration_autocorrect(self, values: Union[np.ndarray, list], motor_names: Optional[List[str]]):
    ...
    def apply_calibration(self, values: Union[np.ndarray, list], motor_names: Optional[List[str]]):
    ...
    def autocorrect_calibration(self, values: Union[np.ndarray, list], motor_names: Optional[List[str]]):
    ...
    def revert_calibration(self, values: Union[np.ndarray, list], motor_names: Optional[List[str]]):
    ...
    def read(self, data_name, motor_names: Optional[Union[str, List[str]]] = None):
        if self.mock:
            import tests.mock_scservo_sdk as scs
        else:
            import scservo_sdk as scs
    ...
    def write(self, data_name, values: Union[int, float, np.ndarray], motor_names: Optional[Union[str, List[str]]] = None):
        if not self.is_connected:
            raise RobotDeviceNotConnectedError(
                f"FeetechMotorsBus({self.port}) is not connected. You need to run `motors_bus.connect()`."
            )
